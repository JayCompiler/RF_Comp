...
import math as mt
#导入整个数据集以及对应的标签
posdataset=np.loadtxt('posk2')
negdataset=np.loadtxt('negk2')
...
clf = RandomForestClassifier(n_estimators=500, max_depth=None,criterion='gini',
     min_samples_split=2, random_state=None,max_features=int(mt.sqrt(dataset.shape[1])),
     n_jobs=-1)
scores = cross_val_score(clf, dataset, label,cv=5,scoring='accuracy')
result=scores.mean() 
print(result) 
